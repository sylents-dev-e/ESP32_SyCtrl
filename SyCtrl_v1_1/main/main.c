...
bool driver_cfg(void) {
	...
	// init spi driver for custom ILI9341 (DEPRECATED)
#if CUSTOM_ILI9341
	#if CONFIG_LCD_OVERCLOCK
		//Clock out at 26 MHz
		spi = init_spi(PIN_NUM_MISO, PIN_NUM_MOSI, PIN_NUM_CLK,480 * 40, 8*1000*1000, 0,
					PIN_NUM_CS, 1, &lcd_spi_pre_transfer_callback, LCD_HOST, DMA_CHAN);
	#else
		Clock out at 32 MHz
		spi = init_spi(PIN_NUM_MISO, PIN_NUM_MOSI, PIN_NUM_CLK,
				PARALLEL_LINES * 320 * 2 + 8, SPI_MASTER_FREQ_40M, 0, PIN_NUM_CS, 1,
				&lcd_spi_pre_transfer_callback, LCD_HOST, DMA_CHAN);
	#endif

	// start display with initial parameter
	lcd_start(spi);
	lcd_draw_string(0, 224, "Init Gear : 0", CYAN, BLACK, spi);
	lcd_draw_string(0, 0, "SW-Version SyCtrl v1.1", RED, BLACK, spi);
#endif
